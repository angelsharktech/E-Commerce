...
export const getProductBySubCategory = async(req,res,next) =>{
  try {
    const result = await product.find({subCategory: { $regex: `^${req.params.name}$`, $options: 'i' }})   //IGNORE CASE SENSITIVITY
    res.status(200).json(result)  
  } catch (error) {
    console.log(error);
...
export const getProductByAllCategory = async(req,res,next) =>{
  try {
    const result = await product.find({mainCategory: req.params.main , subCategory: req.params.sub})   
    res.status(200).json(result)
  } catch (error) {
    console.log(error); 
  }
